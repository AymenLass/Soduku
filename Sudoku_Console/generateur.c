...
void generateur(int grille[9][9][10], int *niveauRequis)
{
    ...
    do
    {
        ...
        // On envoi la grille au backtracking qui va la remplir en entier de mani�re
        // al�atoire
    ...
    while (niveauTeste != (*niveauRequis)); // Tant que la grille n'a pas le
    // niveau d�sir� on recommence
}

/// Proc�dure qui permet de retirer des cases � la grille en fonction du niveau
// requis


void vidage(int grille[9][9][10], int *niveauRequis)
{
    int nombreDeCases = 0, indice2 = 0, *position = NULL, indice = 0,
            niveauMax = 0,
            grille2[9][9][10] = { { { 0 } } }, sauvegarde[9][9][10] = { { { 0 } } },
            niveauTeste = 0, compteur = 0, ligne = 0, colonne = 0;

    // Choose the number of starting cells randomly based on the desired level
    if (*niveauRequis < 3)
    {
    ...
    // Create an array with the positions of the cells randomly
    position = malloc(81 * sizeof(int));
    if (!position)
    {
        // Error handling if memory allocation fails
        perror("Memory allocation failed for position array");
        exit(EXIT_FAILURE);
    }
    ...
    for (compteur = nombreDeCases; compteur < 81; compteur++)
    {
        ...
        /* Tant qu'on a pas une case qui se r�sout avec un niveau de difficult�
           inf�rieur ou �gal � celui d�sir�
           alors on passe � la case suivante du tableau des positions jusqu'� la fin
              de ce dernier */

        // On enregistre le niveau maximum rencontr� lors de la r�solution de la
        // grille
